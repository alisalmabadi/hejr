...
;(function ( $, window, document, undefined ) {
    ...
    var pluginName = 'strength_meter',
        defaults = {
            ...
            showPasswordText: 'نمایش رمزعبور',
            hidePasswordText: 'مخفی کردن رمز عبور'
        };
    ...
    Plugin.prototype._create = function () {
        ...
        this.element.append('<input aria-describedby="url-error" name="password" type="password" class="' + options.inputClass +" "+"form-control m-input" + '" "/>');
        this.element.append('<input aria-describedby="url-error" name="password" type="text" class="' + options.inputClass +" "+ "form-control m-input" + '" style="display:none"/>');
        this.element.append('<a style="text-decoration: none; color:black;" href="" class="' + options.toggleButtonClass + '">' + options.showPasswordText + '</a>');
        ...
        this.element.append(
            ...
            <ul> \
            <li data-criterion="length" class="valid">8 تا 20 <strong>رقم</strong></li> \
            <li data-criterion="capital" class="valid">حداقل<strong>یک حرف بزرگ</strong></li> \
            <li data-criterion="number" class="valid">حداقل <strong> یک عدد</strong></li> \
            <li data-criterion="letter" class="valid">بدون فاصله باشد</li> \
            </ul> \
            </div>');
    ...
    Plugin.prototype._update_indicator = function(total) {
        ...
        if (total === 0) {
            ...
            meter.addClass('veryweak').html('<p>خیلی ضعیف</p>');
        } else if (total === 2) {
            meter.addClass('weak').html('<p>ضعیف</p>');
        } else if (total === 3) {
            meter.addClass('medium').html('<p>متوسط</p>');
        } else {
            meter.addClass('strong').html('<p>قوی</p>');
        }
    },
